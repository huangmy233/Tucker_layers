...
void TuckerReadFactorMatrix(MemoryPackK_t const memory[],
                            Stream<MemoryPackK_t> &pipe,
                            const TuckerConfig &config,
                            const unsigned matrix_id) {
    #pragma HLS INLINE OFF
    
    // Determine matrix dimensions based on matrix_id
    unsigned rows, cols;
    switch(matrix_id) {
        case 0: // U1
            rows = config.in_features;
            cols = config.rank1;
            break;
        case 1: // U2 
            rows = config.in_features;
    ...
    // Burst read factor matrix
    ReadFactorMatrix_Row: for(unsigned i = 0; i < rows; i++) {
        ReadFactorMatrix_Col: for(unsigned j = 0; j < cols; j += kMemoryWidthK) {
            #pragma HLS PIPELINE II=1
            
            // Calculate memory index
            const unsigned index = i * (cols/kMemoryWidthK) + j/kMemoryWidthK;
...
void TuckerReadCoreTensor(MemoryPackK_t const memory[],
                          Stream<MemoryPackK_t> &pipe,
                          const TuckerConfig &config) {
    #pragma HLS INLINE OFF
    
    // Implement double buffering for core tensor
    MemoryPackK_t core_buffer[2][kBufferSize];
    #pragma HLS ARRAY_PARTITION variable=core_buffer complete dim=1
    
    // Read core tensor in blocks
    ReadCore_R1: for(unsigned r1 = 0; r1 < config.rank1; r1++) {
        ReadCore_R2: for(unsigned r2 = 0; r2 < config.rank2; r2++) {
            ReadCore_R3: for(unsigned r3 = 0; r3 < config.rank3; r3 += kMemoryWidthK) {
                #pragma HLS PIPELINE II=1
                
                // Calculate memory index for core tensor
                const unsigned index = (r1 * config.rank2 * config.rank3 + 
                                     r2 * config.rank3 + r3) / kMemoryWidthK;
                
                // Double buffered read
                const unsigned buffer_id = r1 % 2;
                pipe.Push(memory[index]);
            }
        }
    }
...
void TuckerWriteOutput(Stream<MemoryPackM_t> &pipe,
                       MemoryPackM_t memory[],
                       const TuckerConfig &config) {
    #pragma HLS INLINE OFF
    
    // Implement output buffering
    MemoryPackM_t output_buffer[kBufferSize];
    #pragma HLS ARRAY_PARTITION variable=output_buffer cyclic factor=8
    
    // Write results back to memory
    WriteOutput_Batch: for(unsigned b = 0; b < config.batch_size; b++) {
        WriteOutput_Out: for(unsigned o = 0; o < config.out_features; o++) {
            WriteOutput_In: for(unsigned i = 0; i < config.in_features; i += kMemoryWidthM) {
                #pragma HLS PIPELINE II=1
                
                // Calculate output memory index
                const unsigned index = (b * config.out_features * config.in_features +
                                     o * config.in_features + i) / kMemoryWidthM;
...
// Additional helper function for data layout transformation
void TuckerDataLayoutTransform(Stream<MemoryPackM_t> &input,
                              Stream<MemoryPackM_t> &output,
                              const TuckerConfig &config) {
    #pragma HLS INLINE OFF
    
    // Buffer for reordering data
    MemoryPackM_t transform_buffer[kBufferSize];
    #pragma HLS ARRAY_PARTITION variable=transform_buffer cyclic factor=4
    
    Transform_Outer: for(unsigned n = 0; n < config.batch_size; n++) {
        Transform_Inner: for(unsigned k = 0; k < config.in_features; k++) {
            #pragma HLS PIPELINE II=1
            
            // Read input
            MemoryPackM_t val = input.Pop();
            
            // Transform data layout
            Transform_Compute: for(unsigned m = 0; m < kMemoryWidthM; m++) {
                #pragma HLS UNROLL
                transform_buffer[k * kMemoryWidthM + m] = val[m];
            }
            
            // Write transformed data
            output.Push(transform_buffer[k]);
        }
    }
}
